...
\Fn{maximum($nt$)}{
	...
	$busqueda$ $\leftarrow$ nueva \texttt{Busqueda} \\
	$busqueda.mapa$ $\leftarrow$ \texttt{tabla} \\
	$busqueda.contador$ $\leftarrow$ \texttt{0} \\
	$busqueda.max$ $\leftarrow$ \texttt{($""$, 0)} \\
	crear $nt$ threads y pasarles $busqueda$ y la función \texttt{buscador} como punto de inicio \\
